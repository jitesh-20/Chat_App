...
public class UserDAO {
	...
	public boolean login(UserLoginDTO loginDetails) throws SQLException
	{
		try
		{
			...
			/**
			 * if userID is already registered in registered_user table then after execution of above query rs will contain information of that user i.e., only 1 row is there
			 * so if rs contain 1 row i.e., user is valid so return true
			 */
	...
	public void register(UserRegisterDTO registerDetails) throws SQLException
	{
		...
		try
		{
			...
			if(con!=null)
			{
				query="INSERT INTO registered_user (userid,password,username,mobileno,gender) VALUES (?,?,?,?,?)";
				PreparedStatement statement=con.prepareStatement(query);
